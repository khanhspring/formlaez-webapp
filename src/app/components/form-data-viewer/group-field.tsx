...
const GroupField: FC<Props> = ({ section, data = {}, showContentBlocks }) => {
    ...
    return (
        <div className='border border-slate-900/10 dark:border-slate-600 rounded p-3 flex flex-col gap-4'>
            <div>{section.title || 'Untitled group'}</div>
            {
                records.map((record, index) =>
                    <div className='flex gap-2 items-stretch' key={index}>
                        <div className='w-4 border-r border-slate-900/10 dark:border-slate-600'>
                            <span>{index + 1}</span>
                        </div>
